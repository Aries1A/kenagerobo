...
if __name__ == '__main__':
    ...
    low=-1, high=1, size=(num_dizitized**4, env.action_space.n)) #q_rableをランダムに初期化 (ここで学習途中のものloadしたら続きからできるかも)
    ...
    for episode in range(num_episodes): #試行回数分繰り返す
        ...
        print(POST(name="set_action",data=str(action)))
        episode_reward = 0
        ...
        for t in range(max_number_of_steps): #1試行のループ
            ...
            #サーバにa_{t+1}を送信
            print(POST(name="set_action",data=str(action)))
            print(POST(name="set_QStep",data=str(env.step)))
